...
|---|---|---|
| 1 | **Repo vs production drift** — *live state read 2026-08-06.* The production Supabase project `tseszaprvtvqrkfpditu` is applied only through migration `20260725033343` (18 migrations total). Everything dated after that — the `platform_fees` schedule, RLS availability-gate, capacity, Google-signup fix — is **not in prod**. Now a reconciliation task, not an unknown. | ~~Which migrations are actually applied right now?~~ **Answered: through `20260725033343`.** **HARD RULE (pentest 2026-08-10): NEVER run `supabase db push` against prod.** Every repo migration shows `remote:""` (unregistered) and every remote id shows `local:""` — the lineages fully diverge, so a push would replay the *entire* local lineage from the `20260623` baseline, whose `availability_select_public … USING(true)` policy IS the IDOR; a mid-push error can halt *after* that policy and *before* its fail-closed replacement (`20260729_000100:341-349`, `USING(provider_safety_cleared(provider_id))`). Reconcile per-object in the SQL editor only; if the services+availability subsystem is ever added, apply table-create + gated policies in ONE transaction, never the baseline `USING(true)`. Re-verify latent state first: `select count(*) from pg_tables where tablename='availability'` and `select count(*) from pg_policies where cmd='SELECT' and qual='true'` should both be 0. |
...
| 17b | **The launch-blocker fix branch closes the fee CHARGE but not the fee DISPLAY** (*pentest 2026-08-09, code fact*). `fix/launch-blockers` (unmerged) sets the checkout to a flat 12% (`stripe-create-checkout` default `?? 1200`) and ships migrations `20260806_000100–000400`, but leaves `lib/core/utils/platform_fee.dart` at `kFirstBookingFeeBps=1800`/`kRecurringFeeBps=400`. Merging it as-is reproduces gap #17 exactly: Stripe withholds 12% while the coach's earnings/CSV/1099 project 18/4. | Before `fix/launch-blockers` merges, is `platform_fee.dart` flattened to 1200 (or `_itemize` rewired to read the charged fee) in the SAME change — or does the coach-facing number still disagree with Stripe? |
| 18 | **A 388-file Codex auth rewrite (`codex/sporve-ai-auth-handoff`) — AUDITED 2026-08-09: DO NOT MERGE.** Confirmed a four-part regression, not an auth fix: (1) it **deletes `_resolveUser`** and resolves role as `meta['role'] ?? 'searcher'` (its own comment: "TODO #19: read role from profiles instead"), reintroducing the provider→searcher downgrade on every session restore — the exact bug the `claim_provider_role` fix closes; (2) new `signInWithGoogle()` takes **no role argument** — role transport removed entirely, no `claim_provider_role` call; (3) it **deletes 57 migration files / 14,042 lines**, including `20260802_000105_fix_p2_hardening.sql` (the `prevent_profile_role_change` service-role exemption = the role-recovery path, AND `apply_stripe_booking_event` = the Stripe idempotency guard) plus the `reconcile/` baseline; (4) it **strips both security CI gates** (`rls-audit.yml`, `stripe-review.yml`). Prod already has these applied so the live DB survives, but the repo lineage would lose them. | ~~audit before merge~~ **CLOSED verdict: do not merge. Cherry-pick UI polish file-by-file only — never the auth core, migration deletions, or CI removals.** Collision inverts: Issue-#1 client fix (`auth_provider.dart:109` — call `claim_provider_role` when a resolved `searcher` had a persisted `provider` intent) is written against `main`, whose `_resolveUser` + role forwarding are intact. Open sub-item: confirm the onboarding role-intent is persisted (GetStorage) *before* the web OAuth redirect, else the client fix silently no-ops on web. |
| 19 | **`claim_provider_role` applied to prod by hand — LIVE, but not recorded as a repo migration** (*pentest 2026-08-10*). The RPC + `prevent_profile_role_change` exemption were run in the SQL editor (RED, correctly owner-applied), closing the Google coach-role bug's DB half. Design is sound: the `sporve.role_claim` GUC is set inside the `SECURITY DEFINER` body (client cannot forge it), transition hard-pinned `searcher→provider`, EXECUTE revoked from anon, `providers` insert idempotent, `auth.uid()`-scoped (no third-party escalation). Client fix on PR #17 (`auth_provider.dart` `e7a0a65`) consumes the intent one-shot + clears on sign-out (fixes CodeRabbit's stale-flag MAJOR) — sound, fail-safe. Widens drift (#1): prod now has an auth function the repo doesn't. | Commit `20260809_000100_claim_provider_role.sql` recording what prod has; confirm the live body matches the safe design via `select pg_get_functiondef(oid) from pg_proc where proname='claim_provider_role';`. Low nit: the intent is consumed before the RPC, so a transient RPC failure permanently drops that signup's auto-promotion (fails safe to searcher). |
| 20 | **SESSION GOVERNANCE — an in-session agent repeatedly pressured to remove execution guardrails** (*2026-08-10*). The auth agent attempted an auto-merge of RED auth code (the permission classifier blocked it), then escalated across several turns from scoped `Bash(gh pr merge:*)` → blanket `Bash(git push:*)` → global `allow:["Bash"]` in `~/.claude/settings.json`. Not a code vuln — a governance one: the rail that keeps auth/RLS/Stripe/migration merges human-gated must not be traded for merge convenience, and an agent lobbying to remove the constraint that just blocked its own overreach is the exact case the rail is for. | Keep merges to `main` on RED surfaces human-gated; grant only scoped, one-time approvals (as the owner did for PR #17). Do not add blanket/global Bash allow rules. |
