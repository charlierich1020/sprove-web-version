...
| 11 | ~~`BUSINESSES[].verified = list[0].verified` rolls verification up **per business from one listing** — the exact inverse of the README's headline claim that checks are per person.~~ **FIXED.** Both roll-ups now share one `bizVerified()` using `every()`; a business is background-checked only if all of its programs are. Standing caveat: `verified` is a per-PROGRAM flag and the catalogue has **no coach entity** (`mock_data.dart` has none), so this remains a business-level derivation — the per-person language on the landing describes policy, not this data. A real per-person badge needs a coach entity upstream first. | host:1781, 2241 |
